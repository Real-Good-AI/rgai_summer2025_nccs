...
df.sub <- data.table(df.full) 
df.sub <- df.sub[data.table(EIN2 = names(orgs.no.change)), on = .(EIN2)]
df.sub <- df.sub |> drop_na(TOT_REV) |> select(-TAX_YEAR) |> distinct() |> select(EIN2, TOT_REV, LOG_REV, LOG_REV.original)
...
for (ein in names(orgs.no.change)) {
  ...
  # Fill in the values in df.full for matching EIN2 and TAX_YEAR
...
df <- df.full |> select(EIN2, LOG_REV.mean) |> distinct()
...
df.full <- df.full |> mutate(NTEE = replace_na(NTEE, "UNKNOWN"))
