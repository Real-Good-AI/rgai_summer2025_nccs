...
df.full <- df.full |>
  left_join(df |> select(EIN2, TAX_YEAR, TOT_REV, LOG_REV, LOG_REV.original, LOG_REV.mean), 
            by = c("EIN2", "TAX_YEAR"))

# Add in the other information, like NTEE Code, SIZE, location, etc; note this is invariant to year (in theory)
org.info <- df |> select(-TOT_REV, -LOG_REV, -LOG_REV.original, -LOG_REV.mean,-TAX_YEAR, -YEAR, -outlier, -IMPUTE_STATUS) |> distinct(.keep_all = FALSE)
...
# First, let's put in the "imputed" values for orgs that didn't change, by filling in that value
# TODO: above after removing outliers there are some orgs that also just need same value input: "PREPROCESSING/orgs_reported_same_after_outlier_removal.rds"
...
df.sub <- df.sub |> drop_na(TOT_REV) |> select(-TAX_YEAR) |> distinct() |> select(EIN2, TOT_REV, LOG_REV, LOG_REV.original) # one row per org in orgs.no.change with their revenue values 

# Loop over EIN2s in orgs.no.change
start.time <- Sys.time() # ~ 2 minutes
for (ein in names(orgs.no.change)) {
  years.to.fill <- orgs.no.change[[ein]]
  
  # Get the replacement values from df.sub
  fill_values <- df.sub[EIN2 == ein, .(TOT_REV, LOG_REV, LOG_REV.original)]
  ...
  df.full[(EIN2 == ein & TAX_YEAR %in% years.to.fill),
          `:=`(TOT_REV = fill_values$TOT_REV,
               LOG_REV = fill_values$LOG_REV,
               LOG_REV.original = fill_values$LOG_REV.original)]
}
...
```{r}
# Redo the means / centering to exclude the outliers
means <- df |> 
      filter(IMPUTE_STATUS == "original") |>
      mutate(LOG_REV.original = log(TOT_REV)) |> 
      roup_by(EIN2) |> 
      mutate(LOG_REV.mean = mean(LOG_REV.original)) |> 
      ungroup() |> 
      mutate(LOG_REV = LOG_REV.original - LOG_REV.mean) |>
      select(EIN2, LOG_REV.mean) |>
      distinct()
            
# TODO: update the means in df to get the correct imputation
# TODO: above after removing outliers there are some orgs that also just need same value input, "PREPROCESSING/orgs_reported_same_after_outlier_removal.rds"
...
# Now we have to merge df.predictions with df.full, but first replace the outlier revenues with NA so we can replace with imputed values
