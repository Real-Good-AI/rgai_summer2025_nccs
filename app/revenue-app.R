...
handlers("shiny")
plan(multisession)
# Enable progressr to handle messages globally (important for futures)
...
server <- function(input, output, session) {
      ...
      # Reactive dataset that loads only after user selects NTEE
      df <- reactive({
            req(input$ntee)   # wait until the user selects a category
            readMegaDF(input$ntee)
      })
            ...
            showModal(modalDialog(
                  ...
                  tagList(
                        ...
                        p(
                              ...
                              tags$a(
                                    "User Guide",
                                    href   = "https://www.realgoodai.org/nonprofit-tool",
                                    target = "_blank"
                              ),
      ...
      observe({
            ...
            future({
                  with_progress({
                        p <- progressor(steps = 3)  # define total number of progress steps
                        
                        tryCatch({
      
                              # res is a dataframe with the data corresponding to the top 5 nearest neighbors
                              p("Running nn.search()")
                              res <- nn.search(dat = df_data,
                                    category = ntee_cat,
