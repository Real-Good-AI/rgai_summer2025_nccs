...
readMegaDF <- memoise(function(category) {
      message("Reading data from disk…")
      filename <- paste("processed_mega_df_",category, ".rds", sep="")
      readRDS(filename)
})

# This function finds the top 5 nearest neighbors for the current organization
nn.search <- function(dat, category, user.EIN, user.years, user.history, n.predict){
      df <- dat |> mutate(IMPUTED = (IMPUTE_STATUS != "original"))
