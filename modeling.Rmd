...
all.orgs.data <- readRDS("MODEL/nearest_neighbors/all_orgs_data.rds")
all.orgs.data <- all.orgs.data |> filter(EIN2 %in% trouble)
res <- readRDS("MODEL/nearest_neighbors/res_nn.rds") 
res <- res |> filter(ORIGINAL_ORG %in% trouble)

SIGMA.trouble <- SIGMA[trouble]

res.pars <- readRDS("MODEL/nearest_neighbors/res_parameters.rds") |> filter(EIN2 %in% trouble)
```
