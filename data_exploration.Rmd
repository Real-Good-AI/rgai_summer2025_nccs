...
# which EIN2 have more than one entry?
test_table <- core_data_2022 |>
    count(EIN2) |>
    filter(n > 1)

#head(test_table$EIN2)

# get subset of data table only containing duplicated EIN2 rows
core_data_2022 |>
    filter(EIN2 %in% test_table$EIN2)

ein <- "EIN-47-4469864"
# get the duplicated rows -- note that maybe there are times when its more than 2?
subset <- core_data_2022 |> filter(EIN2 == ein)
# returns array of true/false telling you if rows matched on column
bools <- (subset[1,] == subset[2,]) 
bools


bools <- (is_equal(subset[1,], subset[2,]) & is_equal(subset[1,],subset[3,]) & is_equal(subset[2,],subset[3,]))
bools

# indices of columns where there was a mismatch
indices <- which(!bools)

count_list <- rep(0, ncol(core_data_1989))

count_list[indices] <- count_list[indices] + 1

counts_table <- data.table(variable_name = colnames(core_data_2022), counts = count_list)
counts_table

typeof(indices)

# column names of columns with mismatch
colnames(subset[,indices])
